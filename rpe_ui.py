...
class UI:
    ...
    def draw_ui(self, time):
        ...
        draw_image_left_top_ex(*self.pause_pos, 0, 0, .12, *self.pause, 0 if self.pause_line is None else self.pause_line.r)
        if self.combo >= 3:
    ...
    def set_attach(self, line):
        match line.ui:
            case 1:
                self.pause_line = line
            case 2:
                self.combo_num_line = line
            case 3:
                self.combo_line = line
            case 4:
                self.score_line = line
            case 6:
                self.name_line = line
            case 7:
                self.level_line = line
            case _:
                pass