...
from const import WINDOW_WIDTH, WINDOW_HEIGHT, X, Y, X_SCALE, LINE_WIDTH, PERFECT
...
try:
    ...
    NOTE_TEXTURES = tuple([(i[0], i[1]*X_SCALE*const.config["note_size"], i[2] if index == 4 or index == 10 else i[2]*X_SCALE*const.config["note_size"]) for index, i in enumerate(NOTE_TEXTURES)])
    HE_TEXTURES = tuple([(i[0], i[1]*X_SCALE*const.config["note_size"], i[2]*X_SCALE*const.config["note_size"]) for index, i in enumerate(HE_TEXTURES)])
...
class Line:
    def __init__(self, data: dict, index: int):
        ...
        self.above_holds = list(filter(lambda x: x.type == 2, self.above_notes))
        self.below_holds = list(filter(lambda x: x.type == 2, self.below_notes))
        self.above_notes = list(filter(lambda x: x.type != 2, self.above_notes))
        self.below_notes = list(filter(lambda x: x.type != 2, self.below_notes))
    ...
    def draw(self, color=(1, 0.88, 0.66)):
        if self.a > 0:
            draw_line_ex(self.x, self.y, self.r, self.a, LINE_WIDTH, 5.76*WINDOW_WIDTH, color)
    ...
    def draw_notes(self, time):
        for index, note in enumerate(self.above_notes):
            ...
            if note.judge:
                self.HE.append(HE(self.x, self.y, self.r, note.xPosition, time, color=PERFECT if judged[1] else const.GOOD))
            if judged[0]:
                self.combo += 1
                if judged[1]:
                    self.perfect += 1
    ...
    def draw_holds(self, time):
        for index, note in enumerate(self.above_holds):
            if note.floor_position - self.current_floor_position > 3.666667:
                break
            judged = note.draw(self.x, self.y, self.r, self.current_floor_position, time)
            if note.judge:
            ...
            if judged[0]:
                ...
                if judged[1]:
                    ...
                    self.good += 1
        for index, note in enumerate(self.below_holds):
            if note.floor_position - self.current_floor_position > 3.666667:
                break
            judged = note.draw(self.x, self.y, self.r, self.current_floor_position, time)
            if note.judge:
            ...
            if judged[0]:
                ...
                if judged[1]:
                    ...
                    self.good += 1
        self.above_holds = list(filter(lambda n: not n.judged, self.above_holds))
        self.below_holds = list(filter(lambda n: not n.judged, self.below_holds))
...
class Note:
    def __init__(self, data: dict, dire:int):
        ...
        self.can_good = self.type in (0, 2)
        self.offset = const.config["offset"] if self.can_good else 0
    ...
    def draw(self, x: float, y: float, r: float, current_fp: float, time: float):
        ...
        if self.current_fp >= -0.001:
            if self.type == 2 and self.speed > 0 and self.end_time > time:
                ...
                draw_image_bottom_ex(self.x+_cos, self.y+_sin, r+self.r_offset, 0.1, *NOTE_TEXTURES[5+self.HX*6])
            elif self.type != 2:
        ...
        if time >= self.judge_time-self.offset and not self.judge:
            NOTE_SOUNDS[self.type].play()
            self.judge = True
            self.judged = self.type != 2
            return self.judged, -0.08 < self.offset < 0.08 or not self.can_good
        elif time > self.end_time:
            self.judged = True
            return True, -0.08 < self.offset < 0.08 or not self.can_good
        else:
            return False, -0.08 < self.offset < 0.08 or not self.can_good
